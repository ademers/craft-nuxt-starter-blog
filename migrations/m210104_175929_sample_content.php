...
{
    /**
     * @var string private token for testing with Nuxt
     */
    ...
    public function safeDown(): bool
    {
        ...
        $graphQlService->deleteSchema($nuxtSchema);
    ...
    private function addHomeContent(): bool
    {
        if ($homeEntry = Entry::find()->slug('home')->one()) {
            $homeEntry->setFieldValue('siteIntroduction', '<p>This simple Craft CMS project includes this homepage, a blog section, and an about page. You can use it to see Craft in action with the included Twig templates and headless Nuxt 3 front end.</p>');
            Craft::$app->getElements()->saveElement($homeEntry);
        }
    ...
    private function addGqlToken(): bool
    {
        ...
        $token->name = 'Test Nuxt Token';
        $token->accessToken = $this->gqlToken;
        $token->enabled = true;
        $token->expiryDate = null;
        // $token->schemaId = $nuxtSchema->id;
